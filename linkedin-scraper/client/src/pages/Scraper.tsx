...
const Scraper: React.FC = () => {
  ...
  const handleScrape = async () => {
    ...
    setError('');
    try {
      const res = await api.post('/scrape/linkedin-post', { postUrl: url, userId: 'demo' });
      setResult(res.data);
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0f2027] via-[#2c5364] to-[#1c1c1c] text-white flex flex-col">
      ...
      >
        <motion.h2 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-4xl md:text-5xl font-extrabold font-poppins text-transparent bg-clip-text bg-gradient-to-r from-[#43cea2] to-[#185a9d] mb-8 drop-shadow-lg">Scrape LinkedIn Post</motion.h2>
        <motion.input
          type="url"
          value={url}
        ...
        />
        <motion.button whileHover={{ scale: 1.08, rotateY: 8 }} onClick={handleScrape} disabled={loading || !url} className="px-8 py-4 rounded-xl bg-gradient-to-r from-[#43cea2] to-[#185a9d] text-white font-bold shadow-xl transition-all duration-500 hover:scale-105 hover:shadow-3xl text-xl disabled:opacity-50">
